...
function nextQuestion(cq, nq, isPrev) {
  ...
  if (question == 4 && isPrev == false) return;
...
nextBtn.addEventListener("click", () => {
  nextQuestion(question, question + 1, false);
});

pervBtn.addEventListener("click", () => {
  nextQuestion(question, question - 1, true);
});
...
function howit(params) {
  alert("First click on login then click on start quiz");
}
...
function feature() {
  alert("You can play quiz games on this app");
}